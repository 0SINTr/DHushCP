    ...
    if DHCP in packet and packet[DHCP].options:
        ...
        if DHCP_OPTION_ID in option_dict and option_dict[DHCP_OPTION_ID] == dhushcp_id and SESSION_ID_OPTION in option_dict:
            ...
            try:
                ...
                shared_key = respond_key_exchange(iface, session_id, dhushcp_id, private_key, assembled_data)
...
def main():
    ...
    try:
        ...
        stop_event.set()
        cleanup_process(iface, session_id, DHCP_OPTION_ID, private_key, public_key, shared_key_holder)
        sys.exit(0)
